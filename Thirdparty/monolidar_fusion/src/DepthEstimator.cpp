...
namespace Mono_Lidar
{
                                                 ...
                                                 std::shared_ptr<HelperLidarRowSegmentation> lidarRowSegmenter)
    {
        ...
        std::cout << "visibleIndex lidar points size: " << visibleIndex << std::endl;
    ...
    void DepthEstimator::setInputCloud(const Cloud::ConstPtr &cloud, GroundPlane::Ptr &groundPlane)
    {
        ...
        // std::cout << "***********           do_use_ransac_plane           ************" << std::endl;
        ...
        if (_parameters->do_use_ransac_plane)
        {
            ...
            {
                // std::cout << "groundPlane->CalculateInliersPlane, ransac_plane_min_z: " << _parameters->ransac_plane_min_z
                //           << "  ransac_plane_max_z: " << _parameters->ransac_plane_max_z << std::endl;
            ...
            int nSize = _points._points_cs_camera.cols();
            ...
            /***********            保存地面点云           **********/
            // pcl::PointCloud<pcl::PointXYZI>::Ptr pGround(new pcl::PointCloud<pcl::PointXYZI>());
            // for (size_t i = 0; i < _points_groundplane.size(); i++)
            // {
            //     Eigen::Vector3d point = _points_groundplane.at(i);
            //     pcl::PointXYZI pt;
            //     pt.x = point.x();
            //     pt.y = point.y();
            //     pt.z = point.z();
            //     pGround->push_back(pt);
            // }
            // static int cnt = 0;
            // char fileName[128] = {0};
            // sprintf(fileName, "/home/kinggreat24/pc/%d_ground.pcd", cnt++);
            // pcl::io::savePCDFileBinary(fileName, *pGround);
        }
...
#pragma omp parallel for
        ...
        std::shared_ptr<DepthCalcStatsSinglePoint> calcStats)
    {
        ...
        // std::cout << "process feature : " << featurePoint_image_cs.x() << "  " << featurePoint_image_cs.y() << std::endl;
        ...
        // Segment the neighbors due to depth using a histogram
        // std::cout << "Segment the neighbors due to depth using a histogram" << std::endl;
        VecOfVec3d neighborsSegmented;
        std::vector<int> neighborsSegmentedIndex;
        ...
        if ((ransacPlane != nullptr) && (this->_roadDepthEstimator != NULL))
        {
            ...
            // std::cout << "calculate neighbors in a wider area for the road" << std::endl;
            ...
            if (!this->CalculateDepthSegmentationPlane(neighbors, neighborIndicesCut, neighborsSegmented, calcStats, ransacPlane))
