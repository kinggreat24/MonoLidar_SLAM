 ...
 * @LastEditTime: 2021-09-21 22:09:13
 * @Description: 
 */
...
namespace hybrid_vlo
{
    ...
    SparseLidarAlign::SparseLidarAlign(const vk::PinholeCamera *pinhole_model, const _tracker_t &tracker_info)
    ...
    bool SparseLidarAlign::tracking(Frame *reference, Frame *current, Sophus::SE3 &transformation)
    {
        ...
        pointcloud_ref_ = *(reference->GetMagPointCloud());
        ref_image_pyramid_ = reference->GetImgPyramid();
        cur_image_pyramid_ = current->GetImgPyramid();
