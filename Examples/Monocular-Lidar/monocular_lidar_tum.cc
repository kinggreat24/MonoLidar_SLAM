...
void LoadImages(const string &strAssociationFilename, vector<string> &vstrImageFilenamesRGB,
                vector<string> &vstrImageFilenamesLidar, vector<double> &vTimestamps);
...
int main(int argc, char **argv)
{
    ...
    std::cout << "load data size: " << vstrImageFilenamesRGB.size() << std::endl;
    ...
    for (int ni = 0; ni < nImages; ni++)
    {
        ...
        SLAM.TrackMonocularLidar(imRGB, string(argv[3])+"/"+vstrImageFilenamesLidar[ni], tframe);
                ...
                vector<string> &vstrImageFilenamesLidar, vector<double> &vTimestamps)
{
    ...
    while (!fAssociation.eof())
    {
        ...
        if (!s.empty())
        {
            ...
            ss >> t;
            ss >> sLidar;
            vstrImageFilenamesLidar.push_back(sLidar);
        }
    }
}
