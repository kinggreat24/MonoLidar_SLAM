...
namespace ORB_SLAM2
{
                                                                                                                                                                                                  ...
                                                                                                                                                                                                  mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0)
    {
        ...
        if (sensor == System::STEREO || sensor == System::RGBD || sensor == System::MONOLIDAR)
        {
        ...
        if (sensor == System::MONOLIDAR)
        {
            ...
            if (!need_inverse)
                Tcam_ldiar = Converter::toMatrix4d(lidarCameraExtriParam);
            else
                Tcam_ldiar = Converter::toMatrix4d(lidarCameraExtriParam).inverse();
            ...
            eDepthInitMethod depth_init_type = eDepthInitMethod(depth_type);

            // 激光雷达的类型
            std::string lidar_type("");
            fSettings["Lidar.type"] >> lidar_type;
            eLidarSensorType lidar_sensor_type;
            if ("VLP_16" == lidar_type)
                lidar_sensor_type = VLP_16;
            else if ("HDL_32" == lidar_type)
                lidar_sensor_type = HDL_32;
            else if ("HDL_64" == lidar_type)
                lidar_sensor_type = HDL_64;
            else if ("HDL_64E" == lidar_type)
                lidar_sensor_type = HDL_64_EQUAL;
            else if ("RFANS_16" == lidar_type)
                lidar_sensor_type = RFANS_16;
            else if ("CFANS_32" == lidar_type)
                lidar_sensor_type = CFANS_32;
            ...
            mpLidarDepthExtractor = new LidarDepthExtration(depth_init_config, patchwork_param, Tcam_ldiar, depth_init_type, lidar_sensor_type,
            ...
            // 直接法跟踪
            mpSparseLidarAlign = new SparseLidarAlign(mpPinholeCamera, tracker_);
            // mpHybridSparseAlign = new HybridSparseAlign(mpPinholeCamera, tracker_);
        }
    }
    ...
    cv::Mat Tracking::GrabImageMonocularLidar(const cv::Mat &im, const std::string &lidar_files, const double &timestamp)
    {
        mImGray = im;
        ...
        mCurrentFrame = Frame(mImGray, lidar_files, timestamp, mpLidarDepthExtractor, mpORBextractorLeft, mpORBVocabulary, mpPinholeCamera, mK, mDistCoef, mbf, mThDepth);
            ...
            if (!mbOnlyTracking)
            {
                ...
                if (mState == OK)
                {
                    ...
                    if (mVelocity.empty() || mCurrentFrame.mnId < mnLastRelocFrameId + 2)
                    {
                        ...
                        // mpSparseLidarAlign->tracking(&mLastFrame, &mCurrentFrame, Tcl);
                        // mVelocity = Converter::toCvSE3(Tcl.matrix());
                        bOK = TrackReferenceKeyFrame();
                        
                    }
                    else
                    {
                        // Sophus::SE3 Tcl = Converter::toSophusSE3(mVelocity);
                        // mpSparseLidarAlign->tracking(&mLastFrame, &mCurrentFrame, Tcl);
                        // mVelocity = Converter::toCvSE3(Tcl.matrix());
    ...
    void Tracking::MonocularLidarInitialization()
    {
        std::cout << "Monocular Lidar Initialization, feature size: " << mCurrentFrame.N << std::endl;
        if (mCurrentFrame.N > 500)
        {
            // Set Frame pose to the origin
            mCurrentFrame.SetPose(cv::Mat::eye(4, 4, CV_32F));
    ...
    bool Tracking::TrackReferenceKeyFrame()
    {
        ...
        // std::cout << "feature match: " << nmatches << std::endl;
    ...
    bool Tracking::TrackWithMotionModel()
    {
        ...
        int nmatches = matcher.SearchByProjection(mCurrentFrame, mLastFrame, th, mSensor == System::MONOCULAR, vMatches);

        // If few matches, uses a wider window search
        if (nmatches < 20)
        {
            fill(mCurrentFrame.mvpMapPoints.begin(), mCurrentFrame.mvpMapPoints.end(), static_cast<MapPoint *>(NULL));
            nmatches = matcher.SearchByProjection(mCurrentFrame, mLastFrame, 2 * th, mSensor == System::MONOCULAR, vMatches);
        }
        ...
        // cv::Mat match_img;
        // DrawFeatureMatching(mCurrentFrame, mLastFrame, vMatches, match_img);
        // cv::imwrite("feature_match_img.png", match_img);
    ...
    void Tracking::SearchLocalPoints()
    {
        ...
        if (nToMatch > 0)
        {
            ...
            int th = 1;
            if (mSensor == System::RGBD || mSensor == System::MONOLIDAR)
                th = 3;
    ...
    void Tracking::DrawFeatureMatching(const Frame &frame1, const Frame &frame2,
                                       const std::vector<cv::DMatch> &vmatches, cv::Mat &out_img)
    {
        ...
        {
            cv::DMatch dm = vmatches.at(i);

            cv::Point2f pt_1 = frame1.mvKeys[dm.queryIdx].pt;
            cv::Point2f pt_2 = frame2.mvKeys[dm.trainIdx].pt;
